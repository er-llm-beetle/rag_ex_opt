...
class OptimizedDocumentProcessor:
    ...
    def read_document(self, file_path: str) -> Tuple[str, Dict[str, Any]]:
        """Read document with caching and memory optimization"""
        file_path = Path(file_path)
        ...
        if cached_data:
            return cached_data['text'], cached_data['metadata']
        ...
        try:
            ...
            if file_path.suffix == '.pdf':
                content = self._read_pdf_streamed(file_path)
                text = content["text"]
                metadata.update({k: v for k, v in content.items() if k != "text"})
            elif file_path.suffix == '.html':
                text = self._read_html_streamed(file_path)
            elif file_path.suffix in {'.docx', '.doc'}:
                raise NotImplementedError("Not implemented")
            else:
                text = self._read_text_streamed(file_path)
    ...
    def _read_pdf_streamed(self, file_path: Path) -> Dict[str, Any]:
        """Read PDF file in chunks"""
        text_chunks = []
        metadata = {}
    ...
    def _read_text_streamed(self, file_path: Path) -> str:
    ...
    def _read_html_streamed(self, file_path: Path) -> str:
        """Read HTML file with memory optimization"""
        try:
            with open(file_path, 'r', encoding='utf-8') as f:
                parser = BeautifulSoup(f, 'html.parser')
                
            # Remove unnecessary elements
            for tag in parser(['script', 'style']):
                tag.decompose()
                
            # Extract text in chunks
            chunks = []
            for element in parser.stripped_strings:
    ...
    def _clean_text(self, text: str) -> str:
        ...
        if len(text) > self.CHUNK_SIZE:
            ...
            for chunk in chunks:
                cleaned = self._clean_text_chunk(chunk)
                cleaned_chunks.append(cleaned)
                gc.collect()
                
            return ''.join(cleaned_chunks)
        
        return self._clean_text_chunk(text)

    def _clean_text_chunk(self, text: str) -> str:
        """Clean individual text chunk"""
        # Replace tabs
        text = text.replace('\t', ' ')
...
class OptimizedGPTEmbeddingModel:
    """Memory-optimized GPT embedding model with improved caching"""
    ...
    def _load_cache(self):
        ...
        try:
            if self.cache_file.exists() and self.cache_index.exists():
    ...
    def _save_cache(self):
        ...
        try:
            ...
            with open(self.cache_index, 'w') as f:
                json.dump({k: True for k in self._cache}, f)
        except Exception as e:
            self.logger.warning(f"Cache saving failed: {str(e)}")
    ...
    def generate_embeddings(
        ...
        if texts_to_embed:
            # Process in batches
            for i in tqdm(range(0, len(texts_to_embed), self.batch_size), disable=not show_progress):
                ...
                for attempt in range(self.max_retries):
                    try:
                        ...
                        for j, data in enumerate(response.data):
                            ...
                            self._cache[batch_keys[j]] = embedding
...
class ImprovedChromaDBVectorStore:
    """Memory-optimized ChromaDB vector store with better resource handling"""
    ...
    def __init__(
        ...
        # Initialize collection with context manager
        with self._collection_context() as collection:
    ...
    def _load_existing_ids(self):
        """Load existing document IDs from collection"""
        try:
            with self._collection_context() as collection:
                result = collection.get(include=['documents'])
                self._existing_ids = set(result['ids']) if result['ids'] else set()
        except Exception as e:
            self.logger.warning(f"Error loading existing IDs: {str(e)}")
            self._existing_ids = set()
    ...
    def insert(
        ...
        """Insert documents with batching and deduplication"""
        if not texts:
            return
        ...
        try:
            ...
            for i in range(0, len(texts), batch_size):
                ...
                if batch_data:
                    # Insert batch with context manager
                    with self._collection_context() as collection:
                        collection.add(
                            embeddings=[d['embedding'].tolist() for d in batch_data],
                            documents=[d['text'] for d in batch_data],
                    ...
                    self.logger.info(f"Inserted batch of {len(batch_data)} documents")
    ...
    def hybrid_search(
        ...
        """Perform hybrid search with optimized memory usage"""
        try:
            with self._collection_context() as collection:
...
#                 raise
    ...
    def __init__(
            ...
            try:
                ...
                st.write("Initializing document processor...")
                # self.doc_processor = DocumentProcessor()
                self.doc_processor = OptimizedDocumentProcessor(cache_dir=str(self.cache_dir / "docs"))
                log_memory_usage("After document processor init")
                self.logger.info("Document processor initialized")
...
# inference api hf
                ...
                self.embedding_model = OptimizedGPTEmbeddingModel(
                    model_name=embedding_model,
                    api_key=OPENAI_API_KEY,
...
# cache dublicates ?...
                self.vector_store = ImprovedChromaDBVectorStore(
                    collection_name=collection_name,
                    embedding_dim=self.embedding_model.dimension,
                    persist_directory=f"{self.cache_dir}/vector_store"
                )
# 
...
# 
    ...
    def __del__(self):
        ...
        try:
            ...
            Exception('Ragpipeline cleanup problem happened.')
